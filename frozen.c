...
static int get_escape_len(const char *s, int len) {
	switch (*s) {
		case 'u':
			return len < 6 ? JSON_STRING_INCOMPLETE : is_hex_digit(s[1]) && is_hex_digit(s[2]) && is_hex_digit(s[3]) && is_hex_digit(s[4]) ? 5 : JSON_STRING_INVALID;
		case '"':
		case '\\':
		case '/':
		case 'b':
...
static int parse_identifier(struct frozen *f) {
	...
	while (f->cur < f->end && (*f->cur == '_' || is_alpha(*f->cur) || is_digit(*f->cur))) {
		f->cur++;
	}
...
static int parse_value(struct frozen *f) {
	...
	else if (is_digit(ch) || (ch == '-' && f->cur + 1 < f->end && is_digit(f->cur[1]))) {
		TRY(parse_number(f));
	}
	else {
		return ch == END_OF_STRING ? JSON_STRING_INCOMPLETE : JSON_STRING_INVALID;
...
const struct json_token *find_json_token(const struct json_token *toks, const char *path) {
	while (path != 0 && path[0] != '\0') {
		...
		for (i = 0; i < toks[-1].num_desc; i += skip, ind2++) {
			...
			if (ind == -1 && toks[i].type != JSON_TYPE_STRING)
				return 0;
			if (ind2 == ind || (ind == -1 && toks[i].len == n && compare(path, toks[i].ptr, n))) {
				i += skip - 1;
				break;
			};
			if (toks[i - 1 + skip].type == JSON_TYPE_ARRAY || toks[i - 1 + skip].type == JSON_TYPE_OBJECT) {
